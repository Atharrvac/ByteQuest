(function ($) {
  ...
  $(window).on("load", function () {
    ...
    var testimonialsTwoThumbCarousel = null;
    if ($(".testimonials-two__thumb-carousel").length) {
      try {
        testimonialsTwoThumbCarousel = new Swiper(
        ...
        console.error("Swiper Thumb initialization failed:", e);
      }
    }
    ...
    if ($(".testimonials-two__carousel").length) {
      try {
        ...
        var testimonialsTwoCarousel = new Swiper(".testimonials-two__carousel", swiperOptions);
      } catch (e) {
        console.error("Swiper Carousel initialization failed:", e);
      }
    }
    ...
    // Toggle Contrast
    $(document).on('click', '#toggleContrast', function() {
      $('body').toggleClass('high-contrast');
      const isActive = $('body').hasClass('high-contrast');
      localStorage.setItem('accessible-contrast', isActive);
      speak(isActive ? "High contrast mode enabled" : "High contrast mode disabled");
    });

    // Toggle Font
    $(document).on('click', '#toggleFont', function() {
      $('body').toggleClass('large-font');
      const isActive = $('body').hasClass('large-font');
      localStorage.setItem('accessible-font', isActive);
      if(isActive) {
        $('body').css('font-size', '1.2em');
      } else {
        $('body').css('font-size', '');
      }
      speak(isActive ? "Font size increased" : "Font size reset");
    });

    // Restore Settings
    if (localStorage.getItem('accessible-contrast') === 'true') {
      $('body').addClass('high-contrast');
    }
    if (localStorage.getItem('accessible-font') === 'true') {
      $('body').addClass('large-font').css('font-size', '1.2em');
    }
    ...
    if (SpeechRecognition) {
      ...
      $(document).on('click', '#startVoice', function() {
        $(this).addClass('voice-active');
        speak("Voice detection active. You can say Vote for, Register, or Read Instructions.");
        recognition.start();
      });

      recognition.onresult = function(event) {
        const command = event.results[0][0].transcript.toLowerCase();
        $('#startVoice').removeClass('voice-active');
        
        if (command.includes('vote for')) {
          const party = command.split('vote for')[1].trim();
          const link = $(`#vote-${party}`);
          if (link.length) {
            speak("Voting for " + party);
            window.location.href = link.attr('href');
          } else {
            speak("Could not find party " + party);
          }
        } else if (command.includes('instructions')) {
          $('#instruction').click();
          speak("Opening instruction manual");
